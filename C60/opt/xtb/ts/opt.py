...
mol = ml.data.molecule.load('C60_ts_gaussian.log',format='gaussian')
...
geomopt = ml.optimize_geometry(ts=True,
                               model=xtb,
                               initial_molecule=mol,
                               program='gaussian',
                               program_kwargs={'opt_keywords': ['calcfc', 'nomicro','noeigen']}
                               )
...
final_mol.write_file_with_xyz_coordinates(filename='final_ts.xyz')
...
# Calculate frequency
freq = ml.freq(model=xtb, molecule=final_mol, program='gaussian')
