...
traverse_time_list = []
gap_list = []
# uncertain_traj = np.load('uncertain_traj.npy')
for ii in range(400):
    # if not ii in uncertain_traj:
    # forward_traj = ml.data.molecular_database.from_xyz_file(f'{traj_path}/forward_temp298_{ii+1}.xyz')
    # backward_traj = ml.data.molecular_database.from_xyz_file(f'{traj_path}/backward_temp298_{ii+1}.xyz')
    forward_traj = ml.data.molecular_database.load(f'{traj_path}/forward_temp298_{ii+1}.json',format='json')
    backward_traj = ml.data.molecular_database.load(f'{traj_path}/backward_temp298_{ii+1}.json',format='json')
    # forward_label = 'ts'
    # backward_label = 'ts'
    # for imol in range(len(forward_traj)):
